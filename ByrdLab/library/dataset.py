...
class fashionmnist(StackedTorchDataPackage):
    def __init__(self):
        super().__init__('fashionmnist', get_fashionmnist)
...
#                                        rng_pack=rng_pack) # 这里得到的应该是很多数据的index
#         honest_subsets = self.partition.get_subsets(dataset) # 这里利用得到的partition（即index）来得到真实的样本
#
...
#                 self.subsets[node] = honest_subsets[next_pointer] #把样本分给各个节点
#                 next_pointer += 1
#     def __getitem__(self, index):
